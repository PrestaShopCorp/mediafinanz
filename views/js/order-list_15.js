...
var mediafinanz_list = {
    init: function() {
        mediafinanz_list.createCheckboxes();
    ...
    createButton: function() {
        $(document.createElement('a')).addClass('button').attr({'href': '#','id': 'inkasso'}).text(mediafinanz_list.translate('inkasso')).insertAfter($('table.order'));
    },

    createCheckboxes: function() {
        $('table.order tbody tr td:first-child').each(function() {
                id_order = $.trim($(this).parent().children('td').eq(1).text());

                $(this).append($(document.createElement('input')).addClass('noborder').attr({'type': 'checkbox', 'value':id_order, 'name':'orderBox[]'}));
            }
        );
