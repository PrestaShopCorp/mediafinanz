...
class Mediafinanz extends Module
{
	...
	public function hookActionOrderStatusPostUpdate($params)
	{
		...
		if (Configuration::get('PS_OS_MF_REMINDER') == $order_status->id || Configuration::get('PS_OS_MF_LASTREMINDER') == $order_status->id)
		{
	...
	public function changeOrderState($id_order, $id_order_state)
	{
		...
		else
		{
			...
			if ($current_order_state->id != $order_state->id)
			{
				...
				/*
				$templateVars = array();
				$templateVars['{payment_information}'] = Configuration::get();

				if ($history->addWithemail(true, $templateVars))
					return true;
				else
					throw new Exception($this->l('E-mail has not been sent.'));
				*/
			}
			else
				throw new Exception($this->l('Current state of order is same'));
						...
						if ($days_between >= $days_y)
						{
							try
							{
								if ($this->changeOrderState($order['id_order'], $id_order_state_for_lastreminder))
									...
									self::logToFile('Order '.$order['id_order'].
													' - send lastreminder - successful - mail has not been sent ', 'cron');
							}
							catch (Exception $e)
							{
