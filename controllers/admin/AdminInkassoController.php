...
class AdminInkassoController extends ModuleAdminController
{
    ...
    public function displayViewclaimLink($token, $id)
    {
    ...
    public function renderCreateClaimsList()
    {
        ...
        if (count($order_ids) > 0) {
            $list = Db::getInstance()->executeS(
                'SELECT a.`id_shop`, a.`id_order`, a.`id_address_invoice`, a.`date_add`, a.`id_customer`,
            ...
            foreach ($list as &$row) {
                ...
                $row['claim']['overduefees'] = (!isset($claim[$row['id_order']]['overduefees'])) ?
                    (float)Configuration::get('MEDIAFINANZ_OVERDUEFEES') : $claim[$row['id_order']]['overduefees'];
                if ($row['date_add'] != '') {
                    $row['claim']['dateoforigin'] = date('Y-m-d', strtotime($row['date_add'])); //'2015-5-16';
                } else {
                    $row['claim']['dateoforigin'] = '';
                }
                if ($row['date_lastreminder'] != '') {
                    $row['claim']['dateoflastreminder'] = date('Y-m-d', strtotime($row['date_lastreminder']));
                } else {
                    $row['claim']['dateoflastreminder'] = '';
                }
    ...
    public function postProcess()
    {
        ...
        if (Tools::isSubmit('submitCreateClaims')) {
            ...
            $list = Db::getInstance()->executeS(
                'SELECT a.`id_order`, a.`id_shop` FROM `'._DB_PREFIX_.'orders` a LEFT JOIN `'._DB_PREFIX_.
            ...
            foreach ($list as $row) {
                ...
                $claim_to = array(
                    ...
                    'overdueFees' => Tools::ps_round((float)str_replace(',', '.', $claim[$id]['overduefees']), 2),
